...
class TestDPIEstimation:
    ...
    def test_estimate_dpi_pdf_dimensions_method_raises_error_for_non_pdf(self):
        ...
        with pytest.raises(ValueError, match="This method only works with PDF files"):
    ...
    def test_estimate_dpi_page_size_method_without_a4_assumption_raises_error(self):
        """Test that page size method without A4 assumption raises error for images."""
        doc = Document(sample_paths.SAMPLE_TEXT_JPG)
        
        with pytest.raises(ValueError, match="Cannot estimate DPI without size assumptions"):
    ...
    def test_estimate_dpi_with_no_document_loaded_raises_error(self):
        ...
        with pytest.raises(ValueError, match="No document loaded for DPI estimation"):
            doc.estimate_dpi()
    ...
    def test_constructor_with_estimate_dpi_parameter(self):
        ...
        doc3 = Document(sample_paths.SAMPLE_TEXT_PDF, resolution=150, estimate_dpi=True)
        explicit_dpi = doc3.get_metadata('resolution')['resolution']
        assert explicit_dpi == 150

    def test_constructor_estimate_dpi_does_not_break_on_estimation_failure(self):
